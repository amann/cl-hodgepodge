...
(defun s-xml::node (tag attributes values &aux (type s-xml::*default-dom-type*))
  (case type
    (:lxml (if attributes
...
(defun s-xml::get-tag (node &aux (type s-xml::*default-dom-type*)) 
  (case type
    (:lxml (if (consp node)
               (if (consp (car node))
                   (caar node)
...
(defun s-xml::set-tag (node new-tag &aux (type s-xml::*default-dom-type*))
  (case type
    (:lxml (if (consp node)
               (if (consp (car node))
...
(defun s-xml::get-attributes (node &aux (type s-xml::*default-dom-type*))
  ...
  (case type
    ...
    (:sxml (oam::plist<-alist2 (mapcan #'(lambda (v)
                                           (when (and (consp v)
                                             ...
                                             (cdr v)))
                                       (cdr node))))))
(defun s-xml::set-attributes (node attributes &aux (type s-xml::*default-dom-type*))
  (case type
    (:lxml (if (consp node)
  ...
  node)
(defun s-xml::add-attributes (node attributes &aux (type s-xml::*default-dom-type*))
  (let ((attrs (oam::alist<-plist (s-xml::get-attributes node))))
    (map nil #'(lambda (attr)
                 (pushnew attr attrs :key #'car))
         (oam::alist<-plist attributes))
    (s-xml::set-attributes node (oam::plist<-alist attrs)))
  node)

(defun s-xml::get-values (node &aux (type s-xml::*default-dom-type*))
  (case type
    (:lxml (when (consp node)
             (cdr node)))
    (:sxml (remove :@ (cdr node) :key #'(lambda (n) (s-xml::get-tag n))))))
(defun s-xml::set-values (node values &aux (type s-xml::*default-dom-type*))
  (case type
    (:lxml (if (consp node)
               (rplacd node values)
               (error 's-xml-error::immutable-node :node node)))
...
(defun s-xml::insert-values (node values place
                             &aux (type s-xml::*default-dom-type*))
  (case type
    (:lxml (if (consp node)
...
;;;; format.
(defun s-xml::find-child (node child-path)
  ...
  (let* ((node-values (s-xml::get-values node))
         child-node pos)
...
(defun s-xml::delete-child (node child-path)
