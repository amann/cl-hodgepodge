...
(defmethod mul ((a sequence) &rest rest)
  (let ((factor (apply #'* rest)))
    (map (type-of a) (lambda (x)
		     (* x factor))
	 a)))
(defmethod mul ((a number) &rest rest)
  (apply #'* a rest))
    ...
    ((funct (point)
     ...
     (iteration (worst points best n)
       ...
       (let* ((centroid (div (apply #'add points) (length points)))
	      ...
	      (reflexion (add centroid (mul (sub centroid worst) *alpha*))))
	 (cond
	   ((-< best reflexion)
	    (setq worst second-worst
		  best (let ((expansion (add centroid
					     (mul (sub reflexion centroid)
						  *beta*))))
	   ...
	   (t ;(-<= reflexion second-worst)
	    (let ((contraction (add centroid (mul (sub centroid worst) *zeta*))))
	      (cond
