...
(defpackage #:ch.amann-wolowyk.oam-etl-system
  (:use #:cl)
  (:nicknames #:etl-system))
(in-package #:etl-system)

;;;;** Projects
;;;; A project is an instance of the class ETL::PROJECT. It is labeled
;;;; by a name and creates a package (namespace) of this name. Further
;;;; it holds a collection of named tables. For each of these tables an
;;;; external symbol of same name is interned into the project's package.
...
;;;; project and the name of the table with a slash inbetween -- project-
...
(let ((projects (make-hash-table :test #'eq)))
  (defmethod initialize-instance :after ((self etl::project) &key name
                                         &allow-other-keys)
    (let ((name (oam::to-keyword (string-upcase name))))
  ...
  (let* ((table-name (etl::name self))
         ...
         (package (or (etl::package self)
                      (let ((pkg (find-package package-name)))
                        (when pkg
                          (warn "Problematic table name: A package with name ~A already exists. Cleaning package ~:*~A" package-name)
                          (clean-package pkg)
                          pkg))
...
(defun clean-package (package &optional (symbols :all))
  (if (eq :all symbols)
      (do-symbols (sym package) (unintern sym package))
      (map nil #'(lambda (sym) (unintern sym package)) symbols)))
...
(defun etl-system::make-symbol-binder (name whole symbols)
  ...
  (eval `(defmacro ,name (&body body)
           ,(format nil "Return an anonymous function taking ~A argument~:*~Ps which are bound in BODY in respective order to the symbols ~{~S~#[~; and ~:;, ~]~}; The symbol ~S is bound to the whole list of all arguments." (length symbols) symbols whole)
           `#'(lambda ,',symbols
...
(defmacro etl::with-recordset (recordset-name &body body)
  "Apply each row of the record set RECORDSET-NAME to BODY where the symbols naming the columns of the record set are bound to the corresponding value of the row. RECORDSET-NAME must be a symbol in the correct package; it is not evaluated."
...
;;;;       - poor performance and wrong specification: look for another implementation!
